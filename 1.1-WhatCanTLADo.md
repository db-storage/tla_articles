...
Clock时Lamport多个文献里面提到的例子。假设我们要实现一个简单的Clock，它只支持时和分，没有秒。我们抛开与物理时钟之间的关系，只考虑期自身的Hour和Minute的变化。
...
(* Tick可以理解为Clock走了一下，引起inute变化，但是hour不一定变化 *)
