...
```mermaid
graph TD
A["VoteFor(a, b, v) 是否安全?"] --必须要保证-->C["SafeAt(b,v) == \A c \in 0..(b-1) : NoneOtherChoosableAt(b,v)"]
C --"如何保证在整个区间 [0..(b-1)]都成立？"-->D["ShowsSafeAt(Q, b, v): </br> 把[0,b]区间分为三个段: [0，c-1], [c] 和 [c+1,b-1] "]
```
...
(*存在一个Quorum Q，满足：
    ...
    Cond 2.1保证了，如果c不等于-1，必有SafeAt(c,v)
...
```mermaid
graph LR
F2["SafeAt(c,v)"]--蕴含-->E2["区间[0, c-1]内成立"]
F3["VotedFor(a,c,v) 和 <br/> OneValuePerBallot"]--蕴含-->E3["区间[c]内成立"]
F4["由ShowsSafeAt(Q, b, v)定义，<br/> [c+1, b-1] 内不可能选定任何值"]--蕴含-->E4["区间[c+1, b-1]内成立"]
```
