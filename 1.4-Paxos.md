...
关于Paxos和CAP这些理论已经有很多介绍，本文的重点不是Paxos协议本身。而是是顺着Lamport的思路，即如何用形式化的方法，把Consensus => Voting => Paxos这条线给理清楚，这三个部分分别对应概念(目标)、基于归纳法的理论证明和实现。在这个过程中也顺便学习下图灵奖获得者是如何逐层抽象和思考的。

本文假设读者已有Paxos相关基础，知道它解决什么问题，解决的难点在哪(Split Brain、Network partition等)。如果读者完全没有相关背景，请自行在网上找相关资料阅读。
...
# 3. 理解Paxos的归纳法证明基础：Voting.tla

Voting.tla，比Consensus.tla要复杂，而且基本全是理论，阅读有难度。因此需要抓住重点：理解如何用归纳法得出Vote时需要满足的Safe条件 ，尤其是理解**NoneOtherChoosableAt和ShowsSafeAt**以及相关的THEOREM。

Voting.tla里面没有涉及Paxos的Phase1和Phase2，但是它里面的**归纳法推理才是Paxos算法正确性的核心**。理解这个归纳过程，再去理解Paxos就非常自然了。
...
- 常量 Value、Acceptor、 Quorum和Ballot都需要在执行TLC Model Check时预先定义。Quorum是多个合法的Quorum的集合。Ballot是允许出现的Ballot Number，为了防止状态无限化，实际运行时需要限制 Ballot集合里面的元素个数。

- QuorumAssumption类似于assert，即认为任意两个quorum Q1, Q2都是相交的(至少有一个共同元素)。

- THEROM QuorumNonEmpty 可以理解为理论性注释，认为任意quorum Q都不为空。

- votes和maxBal都是function，function的domain都是Acceptor。用于记录每个Acceptor的投票记录以及受到的最大Ballot。参见TypeOK的定义。
...
- VotedFor(a, b, v):  Acceptor a曾经投票给($b$, $v$)
- ChosenAt(b, v): 存在一个Quorum Q，Q里面每个成员都曾经投票给($b$, $v$)
- chosen，注意这个与Consensu.tla里面的定义不同，它是动态构建的，相当于数据库里的 View
- DidNotVoteAt(a, b):  Acceptor a从未给任何 BN = $b$的选举投票
- CannotVoteAt(a, b)： Acceptor a不能再给任何 BN =b的选举投票，因为maxBal[a] >b并且DidNotVoteAt(a, b)。 为什么需要DidNotVoteAt? 因为chosen是计算已有投票获得的，如果以前投过，就已经计算上了。
...
- 推理：如果Value $w$, $w\ne v$, 如果要实现ChosenAt(b, w)，比如有个 Quorum R，R内所有Acceptor都需要给(b, w)投票，但是R必然与 Q有交集，这与前提矛盾。

### SafeAt(b, v)

- 定义： 任意Ballot $c$: $c < b$，NoneOtherChoosableAt(c, v)都成立

### THEOREM AllSafeAtZero

- 这个理论之所以成立，是因为不可能有$<0$ 的Ballot形成决议，因为$<0$的选举，不会被 Accept.

### THEOREM ChoosableThm

- 这个是对"选定"的一个断言：只要ChosenAt(b, v)成立，NoneOtherChoosableAt(b, v)一定成立。
...
- 如果一个Acceptor先后给(b, v) 和 (b, w)投票，那么v一定跟w相等
- 这个实际上隐含了: 给同一Ballot投票多次是允许的，只要Value不变

### OneValuePerBallot

- 如果不同Acceptor给同一Ballot的投票，Value必须相同
...
- 如果能满足TypeOK, VotesSafe和OneVote都成立，那么chosen最多只有一个值，不会选定多个。其中:
  ...
  - OneVote: **同一Ballot**，不会有多个Value被投票，所以不会形成多个的决议

### ShowsSafeAt(Q, b, v)

- 注意这个是每个Acceptor在投票前需要检查的条件
- 存在一个Quorum Q，其中任意一个Acceptor a，满足maxBal[a] >= b ，并且：
- 存在一个小于b的Ballot c，满足下面两个条件：
  - 如果 $c \ne -1$，则满足：Q中至少有一个Acceptor a, a给(b, v)投过票。注意，如果 $c == -1$，则没有限制，这是一个特殊场景，即所有Acceptor都没有投过票。
  -  Q中没有任何Acceptor a，给[c+1, b-1]之间的任何Ballot Number投过票。即Q里面的所有 Acceptor在[c+1, b-1]这个区间，都完全没有投票，只增加了maxBal。
...
###  考虑Voting.tla，它可以做model check，那么OneValuePerBallot 是怎么保证的？

- 参见VoteFor的条件，实际上是检查了所有acceptor的vote，这个是数学抽象，实际上系统中很难原子地检查所有Acceptor的vote。摘录:
...
###   Voting有没有类似于Phase1的操作？

-  IncreaseMaxBal比较接近。但是非常简要，纯数学表示，没有描述消息过程。
...
- 因为它用上帝视角在做vote，VoteFor里面的判断所有Acceptor状态、投票都认为是原子操作。但是实际不可能让别的Acceptor停下来，等待自己干完活。

- Paox把所有的操作看成了异步的，只需要根据消息来运行。
