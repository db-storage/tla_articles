...
```mermaid
graph TD
A["VoteFor(a, b, v) 是否安全?"] --需要满足-->B[" VotedFor(a, b, v) => SafeAt(b,v)，
"]
B--SafeAt定义-->C["其中SafeAt(b,v) == \A c \in 0..(b-1) : NoneOtherChoosableAt(b,v)"]
C --"如何保证在整个区间 [0..(b-1)]都成立？"-->D["把区间分为三个段: [0，c-1], [c] 和 [c+1,b-1] "]
D-->E["c的定义]"]
D-->E2["区间[0, c-1]的含义"]
D-->E3["区间[c]证明"]
D-->E4["区间[c+1, b-1]的证明"]
