...
- 任意2个不同Acceptor给同一Ballot的投票，Value必须相同
...
- 注意这个是每个Acceptor在投票前需要检查的条件，而不是推理
...
- 根据ShowSafeAt(Q, b, v)的定义，我们将Ballot分为2个区间，[-1, c]和[c+1, b-1]。

- 如果 $c == -1$，，则Q内Acceptor都没有投过， SafeAt(b, v)成立。
- 如果$c \ne -1$，则有 VotedFor(a, c, v)成立，而VotesSafe => SafeAt(c, v)成立；而[c+1, b-1]期间根本Q内Acceptor都没有投票，不可能形成决议。因此，SafeAt(b, v)成立。
