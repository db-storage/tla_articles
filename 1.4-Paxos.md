...
关于Paxos和CAP这些理论已经有很多介绍，本文的重点不是Paxos协议本身。而是是顺着Lamport的思路，即如何用形式化的方法，把Consensus => Voting => Paxos这三部曲给理清楚，这三个步分别对应概念(目标)、基于归纳法的正确性推理和协议实现。在这个过程中我们也顺便学习下Lamport是如何逐层抽象和思考的。

本文假设读者已有Paxos相关基础，知道它解决什么问题，解决的难点在哪(知道Split Brain、Network Partition等)。如果读者完全没有相关背景，请自行在网上找相关资料阅读。
...
Voting.tla，比Consensus.tla要复杂，有不少理论。因此需要抓住重点：理解如何用归纳法得出Vote时需要满足的Safe条件 ，尤其是理解**NoneOtherChoosableAt 和 ShowsSafeAt**以及相关的THEOREM。
...
- QuorumAssumption类似于程序里面的assert，即认为任意两个quorum Q1, Q2都是相交的(至少有一个共同元素)。
...
- ChosenAt(b, v): 存在一个Quorum Q，Q 里面每个成员都曾经投票给($b$, $v$)
- chosen，注意这个与Consensu.tla里面的定义不同，它是动态构建的，类似于数据库里的 View的概念
...
先不考虑定理，就讨论model checker可以验证的部分。
...
在决定Vote前，需要先获得一个Quorum的状态，这个状态需要满足：ShowsSafeAt(Q, b,  v)

```
(*存在一个Quorum Q，满足：
    Cond 1 对Q中的任意一个成员a，都有maxBal[a] >= b成立； 
    Cond 2 存在一个c, -1 <= c <= b-1，满足：
      Cond 2.1 如果c 不等于 -1, 那么Q中一定存在一个a，VotedFor(a, c, v)
      Cond 2.2 在[c+1, b-1]这个区间任意Ballot d, Q中的任意成员都没有给d投过票
    
    Cond 1和Cond 2.2 保证了[c+1, b-1]这个区间不可能选定任何值。
    Cond 2.1的定义，如果c不等于-1，则有VoteFor(a,c,v)，这个Vote的前提是SafeAt(c,v)
...
Q内所有成员，在$[0, b-1]$区间都没有Vote过, 且不能再vote（$maxBal[a]>b$)，那么在[0, b-1]区间，不可能选定任何值。

### 3.3.2  $c \ne -1$的场景

Vote过的最大Ballot Number是c。Q内所有成员，在[c+1, b-1]区间都没有Vote过，
