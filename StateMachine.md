# 为什么可以用串行化状态机描述分布式系统？串一下Lampor关于状态机的几篇文章
...
**简单来说**：一个计算过程可以看出一系列的步骤，称为一个行为(behavior)。一个状态机由初始状态集合和行为集合构成。
...
其中，$<s_i, s_{i+1}>$表示从状态$s_i$转变到状态$s_{i+1}$。如果一个状态及的 next-state 关系 $\mathcal{N}$是个函数，即对于任意状态 $s$，只有一个状态$t$，满足$<s, t> \in \mathcal{N}$，则该状态机的是确定性的(deterministic)。非确定性的则是允许从一个状态转变为多个状态。

简单来说，可以把 $ \mathcal{N}$ 理解成输入和输出都属于$ \mathcal{S}$ 的动作。

按照Lamport的观点，**计算/行为才是本质，而不是语言或者形式**。



# 2. 如何把整个分布式计算看成一个状态机？

行为分为有限和无限的，通常讨论的计算行为都是有限的，都会正常结束或者死锁。但是有些行为是无限的，比如一台冯诺依曼计算机，把它看成个状态机，则状态可以认为是无限的。
...
简单来说，分布式计算中，只有少数event之间是真的有因果关系的(Partial Order，偏序/部分有序)，且是确定的。而大部分事件间的顺序并不重要(即使在物理上确实有先后)，在保持偏序关系的前提下，可以把所有的 event 看成一个顺序化的序列，相当于给所有的event做了个排序，这个顺序被称为Total Order。
...
具有因果关系的各个事件间本来就是串行的，在保持因果关系的前提下，把其他无因果关系的事件按照任意顺序排序，都可以变成一个全序(即全部有序)，这个全序就可以对应到一个行为。一个部分序对应的全序可能有多种，但是它们执行的结果是相同的。
...
Partial order 是指那些有因果关系的事件之间的顺序，不能修改。偏序关系具**有非对称、传递性和反自反性**。

而 Total Order(全序) 是指给所有事件都排个顺序。在分布式系统中，这并不容易，所以才有了Lamport的"Time, Clock"一文。

比如，观众在观看上述纪录片时，看到了小华洗脸发生在小明洗脸之前，但是事实上不一定如此，这依赖于如何剪辑影片。我们把最终剪辑的短视频中的每一幕看成一个 event，那么观众看的这个短视频，每一幕之间实际上都是有序的，这就是一个全序。
...
例如，使用Logical Time [1]， 作为Total Order的产生依据：

- Logical Time，作为Partial Order的依据；
